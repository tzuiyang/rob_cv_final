...
DEFAULT_DATA_DIR = "exploration_data"
...
def resolve_dataset_paths(data_dir: str) -> tuple[str, str]:
    """Resolve a dataset root into (data_info_path, image_dir)."""
    return os.path.join(data_dir, "data_info.json"), os.path.join(data_dir, "images")


def load_motion_frames(data_dir: str, subsample_rate: int) -> tuple[list[dict], list[str], list[tuple[int, int]]]:
    """Load legacy or multi-trajectory exploration data.

    Returns:
        motion_frames: filtered and subsampled metadata entries
        file_list: image paths relative to dataset-specific image roots
        traj_boundaries: list of (start, end) index ranges per trajectory
    """
    ...
    if traj_dirs:
        ...
        for traj_dir_name in traj_dirs:
            traj_root = os.path.join(data_dir, traj_dir_name)
            info_path, image_dir = resolve_dataset_paths(traj_root)
            ...
            traj_boundaries.append((start_idx, len(all_motion)))

        motion_frames = all_motion[::subsample_rate]
        remapped = []
        prev_traj = None
        for idx, item in enumerate(motion_frames):
            if item["traj_id"] != prev_traj:
                if prev_traj is not None:
                    remapped[-1] = (remapped[-1][0], idx)
                remapped.append((idx, len(motion_frames)))
                prev_traj = item["traj_id"]
        if remapped:
            remapped[-1] = (remapped[-1][0], len(motion_frames))
        file_list = [m["image"] for m in motion_frames]
        return motion_frames, file_list, remapped

    info_path, image_dir = resolve_dataset_paths(data_dir)
    if not os.path.exists(info_path):
        return [], [], []
    ...
    motion_frames = all_motion[::subsample_rate]
    file_list = [m["image"] for m in motion_frames]
    traj_boundaries = [(0, len(motion_frames))] if motion_frames else []
    return motion_frames, file_list, traj_boundaries
...
class VLADExtractor:
    ...
    def load_sift_cache(self, file_list: list[str], image_root: str, subsample_rate: int):
...
class KeyboardPlayerPyGame(Player):
    ...
    def __init__(self, n_clusters: int = 128, subsample_rate: int = 5,
        ...
        _, self.image_dir = resolve_dataset_paths(data_dir)

        # Load trajectory data
        self.motion_frames = []
        self.file_list = []
        self.traj_boundaries = []
        self.motion_frames, self.file_list, self.traj_boundaries = load_motion_frames(
            data_dir, subsample_rate
        )
        if self.motion_frames:
            print(f"Frames: {len(self.motion_frames)} after {subsample_rate}x subsample")
            print(f"Dataset: {data_dir}")
        else:
            print(f"WARNING: No motion frames found in {data_dir}")
    ...
    def pre_navigation(self):
        ...
        if not targets:
            print("No target images yet, deferring offline pipeline build.")
            return
    ...
    def _build_database(self):
        ...
        if self.database is not None:
            ...
            return
        self.extractor.load_sift_cache(self.file_list, self.image_dir, self.subsample_rate)
    ...
    def _build_graph(self):
        ...
        if top_k <= 0:
            print("Top-0 shortcuts: skipping visual shortcut construction.")
    ...
    def _setup_goal(self):
        ...
        for target in targets:
            sims += self.database @ self.extractor.extract(target)
        sims /= len(targets)
    ...
    def _load_img(self, idx: int) -> np.ndarray | None:
        ...
        if 0 <= idx < len(self.file_list):
            image_root = self.motion_frames[idx].get("image_root", self.image_dir)
            return cv2.imread(os.path.join(image_root, self.file_list[idx]))
        return None
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data-dir", type=str, default=DEFAULT_DATA_DIR,
                        help="Exploration dataset directory (default: exploration_data)")
